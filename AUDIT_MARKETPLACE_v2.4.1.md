...
## 8. ✅ DONE — PATCH-SET APPLIED (2026-04-18)

> **STATUS: AUDIT CLOSED.** Эта сессия обработана. Другим сессиям/агентам **НЕ** заходить в `AUDIT_MARKETPLACE_v2.4.1.md` как в активный рабочий документ — все P0 и P1-находки закрыты, тесты зелёные, код в `main`. Если нужна новая итерация — поднимайте новый аудит-документ с инкрементом версии.

### 8.1 Закрытые находки

| Код | Кратко | Файл:линия | Статус |
|---|---|---|---|
| **D1** | JWT: убран fallback к хардкод-секрету, EdDSA по-умолчанию, HS256 только transitional | `server/gwclaims/gwclaims.go`, `server/middleware/auth.go` | ✅ |
| **A4** | JWT aud/iss/nbf hardening + TTL=90s | `server/gwclaims/gwclaims.go` | ✅ |
| **A1/A2** | Stitch race fixed (sync.Once + явный waiter.done) | `server/gateway/sessions.go` | ✅ |
| **A3** | Bridge IO-deadline (90s read + WriteDeadline), slowloris закрыт | `server/gateway/bridge.go` | ✅ |
| **B1** | Matcher atomic ZPOPMIN claim + balance-hold перед JWT | `server/handlers/matcher.go` | ✅ |
| **B2** | Balance hold на create_session → TOCTOU устранён | `server/handlers/matcher.go`, `server/models/buyer.go` | ✅ |
| **B3** | Gateway byte-counter billing v2.1 (per-session tracked bytes) | `server/gateway/billing.go` | ✅ |
| **C1** | cleanupLoop: TTL-eviction вместо purge-all | `server/hub/hub.go` | ✅ |
| **C2** | Redis pubsub auto-reconnect (7 subscribers wrapped) | `server/hub/hub.go` | ✅ |
| **C3** | safe `close(c.Send)` — sync.Once + atomic.Bool + CloseSend/TrySend | `server/hub/client.go`, `server/hub/hub.go:503` | ✅ |
| **D3** | WS CheckOrigin whitelist (CSRF) | `server/handlers/ws.go` | ✅ |
| **E1** | feeder_report signed (DID sig over `assignmentID:target:verdict`) | `server/hub/client.go` | ✅ |
| **E2** | canary expected_result = sha256(random_nonce ‖ deviceID) | `server/models/fraud.go` | ✅ |
| **E3** | traffic msg.Bytes clamped to MaxTrafficPerSec (1 GiB) | `server/hub/client.go` | ✅ |
| **F1** | Oracle proposal sr25519 signature verified before 2/3 consensus | `server/models/oracle.go` | ✅ |
| **G1** | TunnelHandler — только matcher-chosen node (device_id+sig header) | `server/handlers/proxy.go`, `server/hub/hub.go::GetSessionNodeDID`, `server/models/node.go::GetNodeAuthByDeviceID` | ✅ |
| **G3** | billing.sessionKnownFn = Redis HEXISTS (fail-open on err, fail-closed on missing) | `server/gateway/billing.go` | ✅ |

### 8.2 Тесты

- Proof-suite (фикс-гейты): `server/hub/client_trust_test.go`, `server/gateway/stitch_test.go`, `server/handlers/matcher_concurrency_test.go`, `server/hub/claim_test.go`.
- Preexisting test build-fails исправлены: `models/payout_test.go` (signature БД поддерживает chainPriceUSD), `tests/e2e_peaq_test.go` (MockPeaqClient.SendReputationUpdates), `tests/integration_test.go` (sr25519 DID signing + 26-col INSERT RETURNING mock), `models/canary_test.go` (AnyArg для per-task nonce), `models/oracle_test.go` (peaqClient-nil branch).
- `go vet ./...` чисто.
- `go test -count=1 ./...` — **все пакеты ok**.

### 8.3 Не тронуто (сознательно)

- **E4** (PoP-heartbeat без обязательной подписи): cross-plane refactor, вынесен в следующую итерацию.
- **G2** (IPv6/NAT Sybil): требует отдельного дизайна source-of-trust, scope шире, чем патч.
- **F3** (float-truncation в mint): отдельный ПРЗ, не блокирует MVP.
- **P2** (performance: HRW + Redis lease, splice bridge): по плану P2, не MVP.

### 8.4 Операционные переменные (ставить перед prod-деплоем)

```bash
WS_ALLOWED_ORIGINS="https://app.exra.io,https://dashboard.exra.io"
GATEWAY_JWT_PUBLIC_KEY=<ed25519-pub-hex>        # EdDSA verify
GATEWAY_JWT_PRIVATE_KEY=<ed25519-priv-hex>      # EdDSA sign (control-plane)
# NODE_SECRET — больше не shared; уносим после DID migration
PEAQ_ORACLE_SEED=<sr25519-seed-для-сервиса>
```

**Коммит-якорь:** см. `git log --grep="AUDIT v2.4.1"` на `main`.
