...
if command -v $GO_CMD >/dev/null 2>&1; then
  ...
  ok "Binary пересобран: server/exra-server-linux ($(du -sh exra-server-linux | cut -f1))"
else
  fail "Go не установлен. Установи Go: https://go.dev/dl/ — нельзя деплоить старый бинарник, может быть неправильная архитектура."
fi
